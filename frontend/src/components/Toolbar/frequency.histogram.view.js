...
非H5环境使用以上配置
    ...
    if (!Recorder.LibFFT) {
      throw new Error(
        $T.G("NeedImport-2", [ViewTxt, "src/extensions/lib.fft.js"]),
      );
    }
  ...
  fn.prototype = FrequencyHistogramView.prototype = {
    ...
    draw: function (frequencyData, sampleRate) {
      ...
      for (var iMirror = 0; iMirror < 2; iMirror++) {
        ...
        for (
          var i = 0, xFloat = XFloat, wFloat = 0, x, y, w, h;
          i < lineCount;
          i++
        ) {
          xFloat += spaceFloat;
        ...
        if (set.stripeEnable) {
          ...
          ctx.shadowBlur =
            (stripeShadowBlur == -1 ? set.shadowBlur : stripeShadowBlur) *
            scale;
